...
async updatePestGrammar(grammar: string) : Promise<Result<string[], PestGrammarError[]>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("update_pest_grammar", { grammar }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async parseInput(input: string, rule: string) : Promise<Result<TokenTree | null, PestGrammarError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("parse_input", { input, rule }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
